...
- Multi-frame animation: while a multi-frame row is displayed, `AnimatedAvatarView` wraps the sprite in a `TimelineView(.periodic(by: 0.01))` (display-link driven, auto-pauses when occluded — no Timer/runloop dependency; a Timer.scheduledTimer approach proved unreliable under Swift concurrency). The frame index is a pure function of elapsed time and the CPU-driven interval — `interval = 200ms / max(1, min(20, cpu% / 5))` (RunCat mapping: 5 FPS idle → 100 FPS at full load); `AppEnvironment.latestCPU` (non-@Published, read via a closure so it never triggers view recomputation) feeds the speed; `displayEquals` gating is preserved. All other states stay static.
