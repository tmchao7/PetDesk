...
- Per-row pose import (v1: 专注/摸鱼/休息): Settings lets the user import one pose image per bubble state — working (专注), drinking (摸鱼), sleeping (休息). `PoseCellProcessor.loadCell` chroma-keys and fits each image into a 192×208 cell; `AppEnvironment.importPose(row:from:)` stores the cell and reassembles the sheet via `SpriteSheetGenerator.generate(fromRowCells:)`, falling back to the avatar base cell for rows without a custom pose. Custom poses are session-persisted via the assembled `spritesheet.png`; changing the avatar clears them. This is the only user-facing pose customization path — the whole-atlas import entry (`SpritesheetImportPolicy`) was removed in the v1 trim.
