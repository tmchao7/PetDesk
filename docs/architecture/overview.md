...
- Window dragging: `PetPanel.constrainFrameRect(_:to:)` returns the proposed frame unmodified, so the 500pt window is not confined to the visible-screen area and can be dragged to the upper half on smaller displays. If the window is dragged fully off-screen, the next launch restores and clamps it back into view.
