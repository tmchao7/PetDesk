...
- Pose cell framing: `PoseCellProcessor` keys the background from the four corners, then computes the subject bbox from strong-alpha pixels and keeps the row/column window containing the central 96% of subject mass. This trims scene edges (desks, beds, gradients, watermark noise) so the character is contain-fit and centered in the 192×208 cell instead of being shrunk by full-frame scenery.
