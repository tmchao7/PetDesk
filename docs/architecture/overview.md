...
- User-imported spritesheets: Settings → 导入精灵图 accepts a user-made PNG/WebP atlas (1536×1664, 8 rows × 8 columns of 192×208, transparent background). `SpritesheetImportPolicy` validates format, exact dimensions, alpha channel, and per-row used-frame content (≥50 non-transparent pixels, following hatch-pet's atlas rules); `AvatarRepository.importSpritesheet` persists it to the same `spritesheet.png` used by playback, and the AI pose provider (Plan C) remains available as an opt-in alternative.
