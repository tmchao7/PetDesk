...
`AvatarRepository` validates and downscales imported files before copying them into Application Support. `PetWindowController` owns the `NSPanel`, persistence, screen clamping, and hit-test regions. Interactive dragging suppresses position persistence and environment publication until mouse-up to avoid high-frequency UserDefaults writes and SwiftUI invalidation. Diagnostics retain only the newest 200 sanitized events in memory.
...
- Per-row pose import (v1: 专注/摸鱼/休息): Settings lets the user import pose images per bubble state — working (专注, 1–8 frames), drinking (摸鱼), sleeping (休息). `PoseCellProcessor.loadCell` chroma-keys and fits each image into a 192×208 cell; `AppEnvironment.importPose(row:from:)` uses processed cells only during spritesheet assembly, then stores `[AnimationRow: Int]` frame counts and reassembles via `SpriteSheetGenerator.generate(fromRowFrames:fallbackCell:)` — user frames fill columns 0..<N without programmatic transforms, remaining columns use the avatar base cell (so restart sync recovers the exact frame count); default/AI rows keep the single-cell + micro-transform path. Custom poses persist via `spritesheet.png`; changing the avatar clears them.
- Multi-frame animation (performance-optimized): frames are **pre-sliced once** by `AnimationFrameStore` per (sheet, row, frame-count). The CALayer path calls `preloadCGImages` and retains only `CGImage` frames; the SwiftUI fallback creates the matching `NSImage` wrappers only when it needs them. Playback does no cropping during frame changes. Multi-frame rows (frameCount > 1) play through `PetLayerRenderer`, an AppKit `NSView` with a discrete `CAKeyframeAnimation` on `contents` (idempotent updates: same images/duration/pause does not rebuild the animation; pause freezes layer time via `speed = 0` + `timeOffset`). `PetLayerRendererRepresentable` is the minimal SwiftUI bridge (frames, size, pause, duration only). The CPU-driven frame duration is clamped to **5–30 FPS** (`interval = clamp(200ms / max(1, min(20, cpu% / 5)), 1/30…0.20)`), preventing CPU-positive feedback; `AppEnvironment.latestCPU` (non-@Published, closure-read) feeds the speed even while a manual state pins the displayed base state. Static avatars and one-frame poses keep the low-cost SwiftUI `Image` path; `AnimatedAvatarView`'s `TimelineView(.periodic(by: interval))` remains only as the non-CALayer fallback and honors `isPaused` (no timeline instantiated while hidden/occluded).
...
- Custom-pose state restore: on launch (or after importing a full spritesheet), `AppEnvironment` re-derives per-row custom pose counts by comparing each working/drinking/sleeping frame against the avatar base cell with a small pixel tolerance. It retains only the count and a single downsampled preview; clearing a row temporarily slices remaining frames from the current spritesheet, preventing full-resolution pose cells from remaining resident in the environment.
- Manual state pinning: 专注 (focusing), 摸鱼 (drinkingTea) and 放松 (sleeping) all set a `manualState` override in `AppEnvironment`; while pinned, CPU metrics are still consumed for the moving average, diagnostics, and animation speed, but they cannot change the displayed base state. Idle events still update the latest idle duration for focus pause/sleep semantics, while the pinned display remains stable until the user picks another action. 专注 is display-pinned too: when the timed focus session completes, `handle` forces `baseState = .focusing` so the pet does not drift back to CPU/idle-driven 摸鱼/放松 (cancellation is only ever triggered by switching to 摸鱼/放松, which sets the new pin directly); `cancelFocus()` clears the pin and syncs the latest real idle duration back into the state machine.
- Duration reminders: `AppEnvironment` tracks the continuous duration of the user-pinned state (专注/摸鱼/放松, per-state minute thresholds from Settings) and renders the per-state message template (with `{minutes}`/`{m}` placeholders) via `reminderText(for:minutes:)`. The 专注 threshold is also applied to the manually started `FocusSession`; changing it during an active session resets both the focus-session countdown and reminder elapsed time. At each threshold multiple it sets `snapshot.bubble = .stateDurationReminder(text)`; `PetWindowController` shows that bubble without activating the app, and `AppEnvironment` clears it after the user-configured display seconds (default 10) on the per-second tick. Settings previews the same rendered text with `ReminderPreviewView` (matching `PetBubbleView` styling).
...
- Pose preview memory: Settings keeps **one downsampled 48×52 preview per row** (`AvatarPreviewImageFactory` true-downsampling; `NSImage(cgImage:size:)` only changed display size while retaining full-resolution data). AppEnvironment keeps only per-row frame counts after assembly; full-resolution custom pose cells exist only in a bounded import/reassembly scope. CALayer playback avoids creating `NSImage` wrappers through the CG-only frame-store path.

### Persistence and update cadence

- Slider-driven `petScale` and `animationSpeedMultiplier` changes update the live UI immediately but persist through a 300ms main-actor debounce; `AppEnvironment.stop()` flushes pending values so a quit does not lose the final setting.
- Todo mutations use a separate 300ms debounce and save only the latest snapshot after a burst of edits. Usage-stat writes retain their independent serialized persistence chain.
