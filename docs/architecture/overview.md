...
- Multi-frame animation: `AnimatedAvatarView` runs a `FrameAnimator` (100 Hz Timer, only while a multi-frame row is displayed) that advances the frame index at a CPU-driven interval — `interval = 200ms / max(1, min(20, cpu% / 5))` (RunCat mapping: 5 FPS idle → 100 FPS at full load). `AppEnvironment.latestCPU` (non-@Published, read via a closure so it never triggers view recomputation) feeds the speed; `displayEquals` gating is preserved. All other states stay static.
