...
- Move/copy follows Dropover semantics: `sourceOperationMask = [.copy, .move]`; Finder offers `.move` on the same volume, `.copy` cross-volume, and WeChat/QQ/Mail only accept `.copy`. On a `.move` drop the source delays ~1s then deletes the originals and removes them from the shelf (`draggingSession(_:endedAt:operation:)`) — the delay lets the destination finish reading, avoiding the "意外错误（-8058）" that an immediate source-delete triggers.
