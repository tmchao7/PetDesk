...
- Pose preview memory: Settings keeps **one downsampled 48×52 preview per row** (`AvatarPreviewImageFactory` true-downsampling; `NSImage(cgImage:size:)` only changed display size while retaining full-resolution data). Playback keeps full-resolution frames in `customPoseCells` only. CALayer playback avoids creating `NSImage` wrappers through the CG-only frame-store path.
