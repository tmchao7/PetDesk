...
- `xcodebuild test -only-testing:PetDeskTests/ShelfDragOutTests`：5 tests, 0 failures（先写失败用例确认红，再实现转绿）。
- 全部 `PetDeskTests`：**127 tests, 0 failures**。
- `make lint`：passed。
...
- 拖出到 Finder/微信/QQ 的人工验证未做（headless）；pasteboard 契约已由单测锁定，行为层待 owner 用真实 app 确认。
- `picture.png` 保持未跟踪，禁止提交。
- 后续合并 `fix/shelf-drag-out` 需 owner 批准。

## Next Actions

1. Owner：构建并运行，人工拖出验证 → Finder 桌面（复制/移动两种模式）与微信/QQ。
...
- Commits: `36ebe2b` fix(shelf): enable reliable file drag-out with AppKit row view。
- Working tree: `picture.png` 未跟踪；生成的 `PetDesk.xcodeproj` 未提交；handoff 会话待随本记录提交。
