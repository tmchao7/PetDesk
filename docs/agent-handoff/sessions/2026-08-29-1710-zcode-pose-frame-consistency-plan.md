...
- **2026-08-29 19:15（zcode，Release 轮）**：owner 指示打标签发布。版本 bump `21068af`（project.yml MARKETING_VERSION 2.0.1→2.1.0 + README 两处，沿用 v2.0.1 的 bump 模式）；注释标签 `v2.1.0` 推送触发 `.github/workflows/release.yml` 由 github-actions[bot] 自动构建 Release 并附 CI 构建的 `PetDesk-2.1.0.dmg`（本地 `make-dmg.sh` 产物与 CI 重复，未使用）；已用 `gh release edit` 补充发布说明（历史 release body 均为空）。push main+tag 时 pre-push `make verify` 全绿。注意：tag 推送会自动建 Release，`gh release create` 会因已存在而报错——先 `gh release view` 检查。
