...
- **Per-pose import (专注/摸鱼/休息)**: each bubble state can use its own single pose image imported from Settings (PNG/WebP, solid/transparent background, auto keying). One pose per state is enough — the app derives that state's animation frames programmatically; states without a custom pose keep the avatar's default look. This is the only user-facing pose customization path (whole-atlas import removed in the v1 trim).
