...
- **User-made spritesheet import**: users can generate the whole 8×8 atlas with any online AI (Doubao/GPT/Gemini, Codex Pet tools, etc.) and upload it from Settings. PNG/WebP, exact 1536×1664 with transparent background; the app validates format/dimensions/alpha/per-row content before switching playback to the imported sheet. No API key or cost involved.
