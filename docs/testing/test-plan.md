...
`ShelfDragOutTests` locks the drag-out contract: the pasteboard carries both the real `file://` path (`public.file-url`, Finder) and the legacy `NSFilenamesPboardType` path array (WeChat/QQ), and the source declares `[.copy, .move]` so the system decides move/copy by volume. `ShelfSelectionTests` locks single / Command-toggle / Shift-range selection and the drag set (whole group vs. single row). Cross-app drag-out to Finder/微信/QQ must be verified by hand — XCUITest cannot synthesize a drop into another app.
