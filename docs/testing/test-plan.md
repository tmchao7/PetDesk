...
`ShelfDragOutTests` locks the drag-out contract: the pasteboard carries both the real `file://` path (`public.file-url`, Finder) and the legacy `NSFilenamesPboardType` path array (WeChat/QQ), and the copy/move mask follows the user's picker. Cross-app drag-out to Finder/微信/QQ must be verified by hand — XCUITest cannot synthesize a drop into another app.
